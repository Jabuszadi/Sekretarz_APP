...
async def parse_srt_files(folder_path: str) -> pd.DataFrame:
    ...
    srt_files = await asyncio.to_thread(lambda p: list(p.glob("*.srt")), folder)
    if not srt_files:
    ...
    for file_path in srt_files:
        try:
            raw_text = await asyncio.to_thread(lambda p: Path(p).read_text(encoding='utf-8'), file_path) # Asynchroniczne czytanie pliku
