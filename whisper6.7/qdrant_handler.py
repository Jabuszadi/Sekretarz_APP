...
async def delete_collection_if_exists(collection_name: str) -> bool:
    """
    Deletes the specified Qdrant collection if it exists.
    ...
    if not collection_name:
        logging.warning("Requested deletion of Qdrant collection with empty name. Skipping.")
        return False

    normalized_name = collection_name.strip()
    if not normalized_name:
        logging.warning("Requested deletion of Qdrant collection with whitespace-only name. Skipping.")
        return False
    ...
    try:
        ...
        logging.error(f"❌ Error retrieving Qdrant collections before deletion attempt: {exc}")
        raise

    if normalized_name not in existing_collections:
        logging.info(f"Qdrant collection '{normalized_name}' not found. Nothing to delete.")
        return False

    try:
        await qdrant_client.delete_collection(collection_name=normalized_name)
        logging.info(f"✅ Qdrant collection '{normalized_name}' deleted successfully.")
        return True
    except Exception as exc:
        logging.error(f"❌ Failed to delete Qdrant collection '{normalized_name}': {exc}")
        raise
