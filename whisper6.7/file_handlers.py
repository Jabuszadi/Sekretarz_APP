...
import asyncio # NOWY IMPORT: Do asynchronicznych operacji
...
async def save_uploaded_file_temp(file: UploadFile, job_id: str, temp_dir_obj: tempfile.TemporaryDirectory, temp_dir_path: Path) -> Path:
    ...
    try:
        f = await asyncio.to_thread(open, saved_file_path, "wb")
        chunk_size = 8192
        while True:
        ...
        if f: await asyncio.to_thread(f.close) # Zamknij plik w przypadku błędu
        await asyncio.to_thread(temp_dir_obj.cleanup) # Asynchroniczne sprzątanie katalogu
...
async def save_file_to_temp_and_convert_if_needed(file: UploadFile, job_id: str) -> Path:
    ...
    if file_ext in ['.mkv', '.mp4', '.avi', '.mov', '.flv', '.wmv']:
        ...
        if not await asyncio.to_thread(convert_mkv_to_wav, str(original_file_path), str(wav_file_path)):
...
async def cleanup_temp_dir(job_id: str | None = None):
    """
    ...
    if job_id and job_id in job_temp_storage and job_temp_storage[job_id].get('type') == 'file':
        try:
            ...
            if temp_info and 'temp_dir_obj' in temp_info:
                ...
                await asyncio.to_thread(temp_dir_obj.cleanup) # Użyj asyncio.to_thread
