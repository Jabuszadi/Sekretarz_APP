...
from starlette.concurrency import run_in_threadpool # Dodaj ten import
...
async def ingest_transcription(
    ...
    file_date_str = await get_relevant_date_for_file(original_source_path)
    ...
    transcription_df = await asyncio.to_thread(pd.DataFrame, data_for_df)
    ...
    documents_to_upload = await asyncio.to_thread(chunk_by_duration, transcription_df, duration=chunk_duration, overlap=chunk_overlap)
    ...
    for doc in documents_to_upload:
        ...
        await run_in_threadpool(agent_db.add_transcript_chunk,
            transcript_id,
            chunk_id_val,
            from_time_val,
            to_time_val,
