...
async def ingest_transcription(
    ...
    if not df.empty and 'date' in df.columns:
        # Convert 'date' column to string for consistent metadata handling
    ...
    for doc in documents:
        ...
        # logging.debug(f"Document metadata before upload - date: {doc.metadata.get('date')}, job_id: {doc.metadata.get('job_id')}") # USUNIĘTO
