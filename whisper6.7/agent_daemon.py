...
def wait_for_api(api_url, timeout=60):
    ...
    while time.time() - start_time < timeout:
        try:
            response = httpx.get(api_url + "/health")
            if response.status_code == 200:
                return True
        except httpx.ConnectError: # Zmieniono httpx.exceptions.ConnectionError na httpx.ConnectError
            pass
        time.sleep(1)
    return False
...
class NewFileHandler(FileSystemEventHandler):
        ...
        if file_path.suffix.lower() in {".wav", ".mp3", ".mkv", ".mp4", ".flac", ".ogg", ".m4a"}:
            ...
            for i in range(5): # Spróbuj 5 razy z opóźnieniem
                try:
                    ...
                    logging.warning(f"Błąd dostępu do pliku {file_path.name}, ponawiam próbę w 1 sekundę... (Próba {i+1}/5)")
                    time.sleep(1)
                except Exception as e:
                    logging.error(f"Nieoczekiwany błąd podczas obliczania hashu dla {file_path.name}: {e}")
                    return # Inny błąd, przerywamy

            if filehash is None:
                logging.error(f"Nie udało się obliczyć hashu dla {file_path.name} po wielu próbach, pomijam.")
                return

            if get_file_status(filehash) == "success": # Sprawdź, czy już był sukces
                logging.info(f"Plik {file_path.name} już był przetwarzany poprawnie, pomijam dodawanie do kolejki.")
                return

            logging.info(f"Nowy plik wykryty: {file_path}, dodaję do kolejki.")
    ...
    def process_file(self, file_path: Path, is_retry: bool = False):
        ...
        if current_status == "success":
            logging.info(f"Plik {file_path.name} już był przetwarzany poprawnie, pomijam.")
            return

        # Jeśli plik jest już w bazie z błędem i to nie jest próba retry, pomijamy na razie
        if current_status == "error" and not is_retry:
            logging.warning(f"Plik {file_path.name} miał błąd (status 'error'), będzie retry później.")
            return
...
def process_existing_files(watch_dir):
    logging.info(f"Przetwarzam istniejące pliki w {watch_dir}...")
    for file_path in Path(watch_dir).iterdir():
        if file_path.is_file() and file_path.suffix.lower() in {".wav", ".mp3", ".mkv", ".mp4", ".flac", ".ogg", ".m4a"}:
            ...
            if current_status is None:
                add_file(file_path.name, str(file_path), filehash, "pending", "")
            logging.info(f"Znaleziono istniejący plik: {file_path}, dodaję do kolejki.")
...
def retry_errors(): # Handler już nie jest potrzebny, kolejka jest globalna
    logging.info("Sprawdzam pliki z błędem do ponownego przetworzenia...")
    error_files_db_records = get_files_with_error() # Zwraca (filename, filepath)
    if not error_files_db_records:
        logging.info("Brak plików z błędem do ponownego przetworzenia.")
        return
    logging.info(f"Retry dla {len(error_files_db_records)} plików z błędem...")
    for filename, filepath_str in error_files_db_records:
        ...
        if file_path.exists():
            ...
            if current_status == "error": # Upewnij się, że to nadal jest błąd i że plik istnieje
                logging.info(f"Ponowna próba przetworzenia: {file_path}, dodaję do kolejki.")
                file_queue.put((file_path, True)) # Dodaj do kolejki (Path, is_retry=True)
            else:
                logging.info(f"Plik {file_path} ma status '{current_status}', nie wymaga retry.")
        else:
            logging.warning(f"Plik {file_path} z błędem już nie istnieje na dysku – pomijam.")
            removed = delete_file_by_path(filepath_str)
            if removed:
                logging.info(f"Usunięto rekord pliku {filepath_str} ze statusem 'error', bo fizyczny plik zniknął.")
            else:
                logging.info(f"Brak rekordu do usunięcia dla pliku {filepath_str} – możliwe, że już został wyczyszczony.")
    ...
    while True:
        ...
        while stable_count < 3 and (time.time() - start_wait < timeout_wait_file):
            try:
                ...
                logging.error(f"Plik {file_path} zniknął podczas oczekiwania na stabilność.")
                file_queue.task_done()
                return
            except Exception as e:
                logging.error(f"Błąd podczas sprawdzania rozmiaru pliku {file_path}: {e}")
                time.sleep(1)
                continue
            time.sleep(1)

        if stable_count < 3:
            logging.warning(f"Plik {file_path} nie osiągnął stabilnego rozmiaru, pomijam tę próbę.")
...
def main():
    ...
    observer.start()
    logging.info(f"Monitoring folderu: {WATCH_DIR}")
    ...
    try:
        ...
        worker_thread.join()
        logging.info("Wszystkie procesy daemona zakończone.")

if __name__ == "__main__":
    # logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s') # USUNIĘTO
    # Najpierw czekaj na API - USUNIĘTO TEN BLOK, GDYŻ DAEMON NIE MA WŁASNEGO API
    # if not wait_for_api("http://localhost:8000/"):
    #     logging.error("API nie jest dostępne. Daemon kończy pracę.")
    #     exit(1)
