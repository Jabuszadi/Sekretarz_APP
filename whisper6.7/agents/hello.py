...
def run_test_search():
    ...
    try:
        ...
        if results:
            ...
            total_transcripts = agent_db.execute_read_query("SELECT COUNT(*) FROM transcripts")[0]['count'] # dostosowano do Postgresa
            logging.info(f"Total transcripts in DB: {total_transcripts}")
            
            total_segments = agent_db.execute_read_query("SELECT COUNT(*) FROM transcript_segments")[0]['count'] # dostosowano do Postgresa
