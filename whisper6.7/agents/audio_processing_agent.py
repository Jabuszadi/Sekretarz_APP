...
async def process_audio_and_summarize_tool(
    ...
    try:
        ...
        if summary_delay > 0:
            logging.info(
                f"  [JOB {file_job_id}] Waiting {summary_delay:.1f}s before generating meeting minutes to reduce Gemini load."
            )
            await asyncio.sleep(summary_delay)

        provider_metadata = utils.build_transcription_metadata(transcription_provider, transcription_model)
